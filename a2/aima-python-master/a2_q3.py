...
def run_q3():
    print('*'*50)
    print('*'*50)
    print('          Question 3 with n=30')
    print('*'*50)
    print('*'*50)
    #set variable as a parameter for min_conflicts function
    variables = [x for x in range(30)]
    #run 5 times for loop to satisfy repeatation
    for m in range(5):
        print('-'*80)
        #set neighbors as a parameter for min_conflicts function
        neighbors = [rand_graph(30, 0.1), rand_graph(30, 0.2), rand_graph(30, 0.3), rand_graph(30, 0.4), rand_graph(30, 0.5)]
        for n in range(5):
            #varaible to count the total number of assigned and unassigned
            total_assign = 0
            total_unassign = 0
            #set domains as a parameter for min_conflicts function
            domains = {}
            #set default of domains
            for i in range(30):
                domains.setdefault(i, [])
            #count time start from now
            start_time = time.time()
            #each iteration will expend the domain of each var by one
            for i in range(30):
                for j in range(30):
                    domains[j].append(i)
                ...
                #get solution
                result = backtracking_search(temp_csp,mrv,lcv,forward_checking)
                total_assign = temp_csp.nassigns + total_assign
                total_unassign = temp_csp.unassigns + total_unassign
                #if the domains are not larger enough than go back the iteration and increase the size of domain
                if result == None :
                    continue;
                #check whether the solution satisfy the check funtion in q2
                elif check_teams(neighbors[n],result)==True:
                    #number of team
                    number_teams = i+1
                    ...
                    print('The total number of times CSP assigned and unassigned :', total_assign,total_unassign)
